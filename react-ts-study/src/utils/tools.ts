 ...
 * @LastEditors: zbx
 * @LastEditTime: 2024-04-24 09:19:24
 * @descript: 文件描述
 */
...
export function transferTxtToEmotion(str: string) {
...
export function filterimg(html: string) {
    const temp1 = html.replace(/(<img).*?(=">)/g, '')
    const result = temp1.replace(/(<img).*?(data-code=")/g, "");
    return result
}
export function filterfag(html: string) {
    let newStr = html
        .replace(/<\/?.+?>/g, "")
        // .replace(/ /g, "") //是否过滤空格
        .replace(/("style).*?(">)/g, "")
...
export const formatUrl = {
    getTiezi: (url: string, type = 'android'): string => {
        let urlReg: Regex = /.*?.eastmoney.com\/news,\d+,(\d+).html/
        const id = url && url.match(urlReg) && url.match(urlReg)[1]
        if (!id) return ''

        const androidUrl = `dfcft://router/news/gubaDetail?postId=${id}&fromPage=PrivateMsg&fromList=index`
        const iosUrl = `eastmoney://page/gubacontent?postid=${id}&posttype=0&fromPage=PrivateMsg&fromList=index`
        return type === 'ios' ? iosUrl : androidUrl
    },
    getArticle: (url: string, type = 'android'): string => {
        let urlReg: Regex = /.*?caifuhao.eastmoney.com\/news\/(\d+)/

        const id = url && url.match(urlReg) && url.match(urlReg)[1]
        if (!id) return ''

        const androidUrl = `dfcft://router/news/cfhDetail?artCode=${id}&fromPage=PrivateMsg&fromList=index`
        const iosUrl = `eastmoney://page/cfhh5?artCode=${id}&fromPage=PrivateMsg&fromList=index`
        return type === 'ios' ? iosUrl : androidUrl
    },

    getZixun: (url: string, type = 'android'): string => {
        let urlReg: Regex = /.*?finance.eastmoney.com\/a\/(\d+).html/
        console.log('-match>', url.match(urlReg))

        const id = url && url.match(urlReg) && url.match(urlReg)[1]
        if (!id) return ''

        const androidUrl = `dfcft://router/news/detail?id=${id}`
        const iosUrl = `eastmoney://page/newsdetail?newsid=${id}&newstype=1`
        return type === 'ios' ? iosUrl : androidUrl
    },
    getZhuanti: (url: string, type = 'android'): string => {
        let urlReg: Regex = /.*?topic.eastmoney.com\/detail\/([a-zA-Z0-9]+).html/
        console.log('-match>', url.match(urlReg))

        const id = url && url.match(urlReg) && url.match(urlReg)[1]
        if (!id) return ''

        const androidUrl = `dfcft://emrn?id=zhuanti&page=homeV2&topicname=${id}`
        const iosUrl = `eastmoney://page/emrn?id=zhuanti&page=homeV2&topicname=${id}`
        return type === 'ios' ? iosUrl : androidUrl
    },
    getTopic: (url: string, type = 'android'): string => {
        let urlReg: Regex = /.*?htid=(\d+).*?/
        console.log('-match>', url.match(urlReg))

        const id = url && url.match(urlReg) && url.match(urlReg)[1]
        if (!id) return ''

        let mUrl = `https://mgubatopic.eastmoney.com/list?id=${id}`
        mUrl = encodeURIComponent(mUrl)
        const androidUrl = `dfcft://router/h5?titleBarType=trans&url=${mUrl}`
        const iosUrl = `eastmoney://wireless/h5/nonavweb?url=${mUrl}&emthemenavmode=1`
        return type === 'ios' ? iosUrl : androidUrl
    },

    getH5: (url: string, type = 'android'): string => {
        url = encodeURIComponent(url)
        const androidUrl = `dfcft://router/h5?titleBarType=trans&url=${url}`
        const iosUrl = `eastmoney://wireless/h5/nonavweb?url=${url}&emthemenavmode=1`
        return type === 'ios' ? iosUrl : androidUrl
    },
}
...
export const extractLinks = (text: string) => {
    const urlReg = /(http|https|dfcft|eastmoney):\/\/([\w=\?\.\/&-]+)/g;
    if (!text || !text.match(urlReg)) return text
    // console.log('-match>', text.match(urlReg))
    // console.log('-match2>', text.match(urlReg))
    // console.log('-exec>', urlReg.exec(text))
    // console.log('-exec2>', urlReg.exec(text))
    // console.log('-exec3>', urlReg.exec(text))
    const result = text.replace(urlReg, function ($url) {
        return "<a href='" + $url + "' target='_blank' class='alink'>" + $url + "</a>";
    });
    return result;
}

extractLinks('啊手动dd阀手动阀https://passport2.eastmoney.com发顺丰到付https://passport2.eastmoney.com/pub/login啊手动dfs阀')
